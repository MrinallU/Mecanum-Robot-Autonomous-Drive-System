...
public class Drive extends Base {
    ...
    public Drive(Motor fLeftMotor, Motor bLeftMotor, Motor fRightMotor, Motor bRightMotor, Motor odoL, Motor odoR, Motor odoN,
                 BNO055IMU gyro, OpMode m, int xPos, int yPos, int angle, List < LynxModule > allHubs) {
    ...
    public void traversePath(ArrayList < Point > wp, double heading, double driveSpeedCap, double powLb, double xError, double yError, double angleError, int lookAheadDist, double timeout) {
        ...
        while ((lastLhInd < wp.size() - 1 || (Math.abs(odometry.getX() - wp.get(wp.size() - 1).xP) > xError ||
                ...
                Math.abs(heading - odometry.getAngle()) > angleError)) && time.milliseconds() < timeout) {
            resetCache();
            updateOdometry();
            ...
            for (Point p: wp) {
                ...
                if (Math.abs(ptDist) <= lookAheadDist && i > lastLhInd && Math.abs(ptDist) > maxDist && Math.abs(i - lastLhInd) < 5) {
                    nxtP = p;
                    possInd = i;
            ...
            double relAngToP = Angle.normalizeRadians(splineAngle - (Math.toRadians(theta) - Math.toRadians(90))); // mtp 2.0
